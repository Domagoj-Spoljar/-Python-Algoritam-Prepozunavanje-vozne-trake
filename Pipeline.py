...
def main():
    ...
    while k is not 27:
        ...
        #imgOriginal = cv2.imread('/home/profesor/Documents/test_files/roma/BDXD54/IMG00106.jpg')               # open image
        ...
        veca_slika=np.copy(imgOriginal)

        # small_org=cv2.resize(imgOriginal,(int(cols/2),int(rows/2)))
        # red,stup,kanal = small_org.shape
        # veca_slika[red:rows,0:stup]=small_org



        cv2.putText(exampleImg_unwarp, 'ROI Warped', (250, 100), 2, 3, (0, 0, 255), 2, cv2.LINE_AA)
        resized_warped=cv2.resize(exampleImg_unwarp,(int(cols/3),int(rows/3)))
        red2,stup2,kanal2 = resized_warped.shape
        veca_slika[0:red2,0:stup2]=resized_warped
        ...
        cv2.putText(res, 'HLS L + LAB B', (250, 100), 2, 3, (0, 0, 255), 2, cv2.LINE_AA)
        resized_warped_lines=cv2.resize(res,(int(cols/3),int(rows/3)))
        red3,stup3,kanal3 = resized_warped_lines.shape
        veca_slika[0:red3,stup2:stup2+stup3]=resized_warped_lines
