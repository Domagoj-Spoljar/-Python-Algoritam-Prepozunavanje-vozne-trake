...
def calculate_IOU(processed_image,processed_lane_inds,gt_image,gt_lane_inds):
...
def main():
    ...
    # line=f.readline()
        ...
        iou_calculations=calculate_IOU(processed_image,lane_count,gt_image,gt_lane_inds_field)

        completeName = os.path.join(save_folder+folder_name+'/', "validation-"+str(readable_time)+".txt")
        ff= open(completeName,"a")
        print(completeName)
        ...
        write_line=compare_folder_path+' '+save_folder_path+' '+gt_lane_inds+' | '+str(lane_count[0])+' '+str(lane_count[1])+' '+str(lane_count[2])+' '+str(lane_count[3])+' | '+str(iou_calculations[0])+' '+str(iou_calculations[1])+' '+str(iou_calculations[2])+' '+str(iou_calculations[3])+'\n'
        print(write_line)
